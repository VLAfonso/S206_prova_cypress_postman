...
O objetivo principal é o desenvolvimento de testes de UI para o website [XYZ Bank](https://www.globalsqa.com/angularJs-protractor/BankingProject/#/login)<!-- e de API para [Nome da API]()-->.
...
<!--
Foram desenvolvidos testes de API na [Nome da API]() realizando requisições (adicionar requisições) utilizando a ferramenta `Postman` e o `Newman` para a geração de relatório.
-->
...
3. Executar os testes
```bash
newman run ProvaS206_Lanna_Virginia.postman_collection.json -e ProvaS206_Lanna_Virginia.postman_environment.json -r htmlextra
```
<!--
Ajustar nome do arquivo conforme nome da coleção e do enviroment
-->
4. Acessar relatório de testes  
...
| 2 | **Teste de adição de cliente com falha** | Negativo | Realiza a tentativa de adição de um cliente sem informar as informações necessárias. |
| 3 | **Teste de abertura de conta com sucesso** | Positivo | Realiza de forma correta a abertura de uma conta.|
| 4 | **Teste de abertura de conta com falha** | Negativo | Realiza a tentativa de abertura de uma conta sem informar as informações necessárias.|
